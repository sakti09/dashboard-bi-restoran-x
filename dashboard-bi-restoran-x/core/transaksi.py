...
# Seluruh fitur per nota yang dihasilkan pembangun fitur (build_transaksi_features) dan
# dapat dipilih untuk eksplorasi: gabungan fitur dasar/klasik dan fitur rekayasa.
# Urutan menempatkan tiga fitur rekomendasi penelitian lebih dahulu.
CANDIDATE_FEATURES = ["total_net", "rasio_minuman", "n_item_unik",
                      "total_qty", "total_gross", "qty_makanan", "qty_minuman", "jam"]
# Fitur besaran/cacah yang layak diwinsor p99 (rasio_minuman [0,1] & jam [0,23] dikecualikan).
WINSOR_CANDIDATES = {"total_net", "total_gross", "total_qty",
                     "n_item_unik", "qty_makanan", "qty_minuman"}
...
def prep_features(nota_df: pd.DataFrame, features):
    """Praproses ruang fitur sembarang: winsorize p99 untuk fitur besaran/cacah
    (rasio_minuman & jam dikecualikan), lalu Z-score. Mengembalikan (Xz, scaler)
    sehingga matriks fitur dapat dipakai ulang untuk metrik per-K maupun klaster final."""
...
def run_custom_clustering(nota_df: pd.DataFrame, features, k: int,
    ...
    (eksplorasi pemilik/pengembang). Fitur besaran/cacah diwinsor p99, sedangkan
    rasio_minuman & jam dibiarkan apa adanya, lalu seluruhnya distandardisasi
    (Z-score) sebelum K-Means(++). Mengembalikan (nota_berlabel, hasil, Xz, scaler)."""
    from core import cluster as cl
...
def build_nota_items(df: pd.DataFrame) -> pd.DataFrame:
    ...
    terbaca, bukan sekadar nomor nota. Mengembalikan DataFrame [nota, items] dengan
    items berupa rangkaian 'qty× nama item' (diurut kuantitas menurun) dipisah '; '."""
    d = filter_valid_rows(df)

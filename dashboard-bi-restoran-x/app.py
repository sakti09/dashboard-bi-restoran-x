...
# Pulihkan peran dari query param bila session_state ter-reset. Hal ini terjadi saat
# kartu modul di halaman Beranda diklik: tautan tersebut memicu navigasi penuh (reload),
# sehingga session_state hilang dan role kembali None. Dengan menyimpan role di URL,
# klik kartu di halaman bekerja sama seperti klik menu di sidebar (tanpa error).
if role is None:
    qp_role = st.query_params.get("role")
    if qp_role in ("pemilik", "developer"):
...
with st.sidebar:
    ...
    if role is not None:
        ...
        if st.button("↩  Ganti peran", use_container_width=True, key="switch_role"):
            ...
            st.query_params.clear()  # buang role dari URL agar kembali ke pemilih peran

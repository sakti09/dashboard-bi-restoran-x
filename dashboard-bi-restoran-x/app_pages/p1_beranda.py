...
def module_card(icon, title, desc, href, role=None):
    # Sertakan role pada URL agar navigasi penuh (reload) tetap mengenali peran aktif,
    # sehingga halaman modul tidak error saat kartu diklik dari halaman Beranda.
    if role:
        sep = "&" if "?" in href else "?"
        href = f"{href}{sep}role={role}"
...
if role is None:
    ...
    with c2:
        ...
        if st.button("Masuk sebagai Developer", key="pick_dev",
            ...
            st.query_params["role"] = "developer"
            st.rerun()
    ...
    if role == "pemilik":
        ...
        cards = [
            module_card(IC["chart"], "Developer &middot; Menu",
                        ...
                        "heatmap korelasi, dan pemilihan kombinasi fitur.", "developer-menu", role),
            module_card(IC["chart2"], "Developer &middot; Transaksi",
                        "Eksperimen teknis klaster transaksi: Elbow/Silhouette/DBI, PCA "
                        "2D/3D, korelasi, dan pemilihan kombinasi fitur.", "developer-transaksi", role),
        ]

...
page_header("Klaster Menu",
            "Segmentasi menu makanan & minuman dengan K-Means++. Atur kelompok, klaster, dan kategori, lalu unduh hasilnya.")

df = ensure_data_loaded("Modul Klaster Menu")
menu = build_menu_features(df)

# ---- pilihan kelompok & skema ----
top = st.columns([1, 2])
...
feat_cols, do_profile = FEATURE_SCHEMES[scheme_name]
...
k = THESIS_K[grup] if do_profile else cluster.best_k_by_silhouette(Xz, 2, 6)
...
for c in clusters_avail:
    ...
    col = charts.OLIVE_SEQ[c % len(charts.OLIVE_SEQ)]
...
color_col = "profil" if do_profile else "cluster"
fig = charts.scatter_clusters(
    view, x=feat_cols[0], y=feat_cols[1], color_col=color_col,
    title="Sebaran Menu per Klaster",
...
download_button(tbl, "Unduh hasil klaster (CSV)",
                f"klaster_menu_{grup}.csv", key="dl_klaster")

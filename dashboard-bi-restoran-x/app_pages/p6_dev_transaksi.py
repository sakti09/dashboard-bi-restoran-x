...
feats = c[1].multiselect(
    "Fitur klaster (pilih 1..maks; gabungan fitur dasar & rekayasa)",
    trx.CANDIDATE_FEATURES, default=list(trx.TRX_FEATURES),
    format_func=lambda f: trx.FEATURE_LABEL.get(f, f))
st.caption("Rekomendasi fitur terbaik (penelitian, hasil uji VIF — semua di bawah 5): "
           "Nilai belanja (net) + Rasio minuman + Ragam jenis item. Kombinasi lain boleh "
           "dieksplorasi untuk kepentingan pengembang; mutu klaster yang dihasilkan dapat "
           "lebih rendah dan tidak diberi pelabelan segmen.")
...
# ---- tabel hasil (sertakan rincian item tiap nota) ----
...
cols_order = ["nota", "items", "cluster", "total_net", "total_qty", "n_item_unik",
